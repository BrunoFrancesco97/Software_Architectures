...
def add_solution(exercise: int, answer: str, user: str, correct):
...
def get_solutions_by_name_and_exercise(user: str, exercise):
    session = database.Session()
    solutions = session.query(Solution).filter_by(user=user, exercise=exercise).all()
    session.flush()
    return solutions
